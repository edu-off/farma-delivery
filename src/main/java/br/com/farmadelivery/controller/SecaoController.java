...
public class SecaoController {
    ...
    private ResponseEntity<String> cadastra(@PathVariable Long farmaciaDocumento, @Valid @RequestBody Secao secao) {
        secaoService.cadastra(farmaciaDocumento, secao);
        return ResponseEntity.ok("seção cadastrada com sucesso");
    }

    @PostMapping("/altera/{id}")
    private ResponseEntity<String> altera(@PathVariable Long id, @Valid @RequestBody Secao secao) {
